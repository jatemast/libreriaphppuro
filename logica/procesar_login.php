...
if (isset($response_data['token']) && isset($response_data['user'])) {
    ...
    $_SESSION['user_role'] = $response_data['user']['role'] ?? 'guest'; // Asume 'role' de la API, por defecto 'guest'

    echo "<script>
        alert('¡Bienvenido, " . htmlspecialchars($_SESSION['user_name'], ENT_QUOTES, 'UTF-8') . " (Rol: " . htmlspecialchars($_SESSION['user_role'], ENT_QUOTES, 'UTF-8') . ")!');
